...
try:
    data = pd.read_csv(general_dir_name+data_dir+file_dir+"").drop(['Unnamed: 0'],axis=1)
    train_set = data.iloc[:,:l].values.tolist()
    u_set = [l//2]*len(train_set)
except:
    print('NO DATA FOUND')
len(train_set)
len(u_set)
...
for i in range(100): 
    model_mppo.learn(total_timesteps=TIMESTEPS, progress_bar=True,tb_log_name='M_PPO_k2', reset_num_timesteps=False, log_interval=1)
    model_mppo.save(f"{general_dir_name+models_dir+version_dir}/k2/{model_mppo._total_timesteps}")
